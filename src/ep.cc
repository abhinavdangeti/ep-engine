...
bool EventuallyPersistentStore::compactVBucket(const uint16_t vbid,
    ...
    if (vb) {
        ...
        if (config.isBfilterEnabled()) {
            ...
            vb->initFilter(estimated_count, config.getBfilterFpProb(),
                           BFILTER_COMPACTING);
        }
        ...
        if (getRWUnderlying(vbid)->compactVBucket(vbid, ctx, cb, kvcb)) {
            if (config.isBfilterEnabled() &&
                    vb->getFilter() &&
                    vb->getFilter()->getStatus() == BFILTER_COMPACTING) {
                vb->getFilter()->enable();
            } else {
                vb->clearFilter();
            }
            ...
            if (v) {
                ...
                if (v->isNewCacheItem()) {
                    if (value.second) {
                        ...
                        /**
                         * Only in the case of FULL EVICTION MODE,
                         * if resident ratio is found to be less than
                         * then threshold add the key that was created
                         * to the bloomfilter.
                         */
                        if (store->getItemEvictionPolicy() == FULL_EVICTION) {
                            float threshold =
                                        store->getBfiltersResidencyThreshold();
                            if (vbucket->isResidentRatioUnderThreshold(
                                                threshold, FULL_EVICTION)) {
