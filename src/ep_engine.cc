...
extern "C" {
                                          ...
                                          uint16_t vbucket)
    {
        ENGINE_ERROR_CODE err_code =
            getHandle(handle)->tapNotify(cookie, engine_specific,
                                         nengine, ttl, tap_flags,
                                         (uint16_t)tap_event, tap_seqno,
                               ...
                               const item* itm, item_info *itm_info)
    {
        ...
        itm_info->datatype = *(it->getExtMeta());
    ...
    case TAP_MUTATION:
        {
            ...
            if (!tc) {
                ...
                ret = ConnHandlerMutate(tc, k, cookie, flags, exptime, cas, datatype,
                                                          ...
                                                          uint16_t nmeta)
{
    ...
    if (consumer->isValidOpaque(opaque, vbucket)) {
        ...
        ret = ConnHandlerMutate(consumer, k, cookie, flags, expiration, cas,
                                datatype, revSeqno, vbucket, true, value, nvalue,
                                meta, nmeta);
    } else {
        ret = ENGINE_FAILED;
    }
